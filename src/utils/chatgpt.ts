...
export const OpenAIModel = async (data: string): Promise<any> => {
    const prompt = `
...
${data}`;
    ...
    try {
        if (process.env.NODE_ENV === 'development') {
            logForDev('[OpenAI] Sending prompt:', { prompt });
        }
        ...
        if (process.env.NODE_ENV === 'development') {
            logForDev('[OpenAI] Raw response:', { response: JSON.stringify(response) });
        }
